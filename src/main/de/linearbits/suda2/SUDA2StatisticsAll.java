 ...
 */
public class SUDA2StatisticsAll extends SUDA2Result {

    private final SUDA2Scores scoreCalculator;
    /** Intermediate scores */
     ...
     * @param maxK
     */
    protected SUDA2StatisticsAll(int rows, int columns, int maxK, boolean sdcMicroScores) {
        ...
        this.scoreCalculator = new SUDA2Scores(columns, maxK, sdcMicroScores);
        this.intermediateScores = scoreCalculator.getIntermediateScores();
        this.scores = new double[rows];
    ...
    public boolean equals(Object obj) {
        ...
        SUDA2StatisticsAll other = (SUDA2StatisticsAll) obj;
    ...
    void registerKey(SUDA2Item item, SUDA2ItemSet set) {
        ...
        this.totalScore += score;
        int min = item.getRows().min();
        int max = item.getRows().max();
        for (int i = 0; i < size; i++) {
            int column = set.get(i).getColumn();
            this.columnKeyContributions[column] += score;
            this.columnKeyTotals[column] += set.size() + 1;
            this.columnKeyCounts[column]++;
            min = Math.max(min, set.get(i).getRows().min());
            max = Math.min(max, set.get(i).getRows().max());
        }
        ...
        if (min != max) {
            SUDA2Item temp = item;
            for (int i = 0; i < size; i++) {
                if (temp.getRows().size() != 1) {
                    temp = temp.getProjection(set.get(i).getRows(), min, max);
                }
            }
            row = temp.getRows().min() - 1;
        }
    ...
    void registerKey(SUDA2ItemSet set) {
        ...
        int row = set.get(0).getRows().min() - 1;
        this.scores[row] += score;
